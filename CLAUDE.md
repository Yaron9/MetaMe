...
当用户提到「上次」「之前」「以前」，或问到某个技术决策/曾经踩过的坑，在回答前先查记忆：

```bash
node ~/.metame/memory-search.js "<关键词>"
```

返回 JSON 数组，每条记录有 `type`（fact/session）、`value` 或 `summary`、`date`。
