...
三个开箱即有的任务。都带前置条件守卫，只在有意义时运行：

```yaml
- cognitive-distill   # 4h · 有信号才触发 → 蒸馏偏好更新画像
- memory-extract      # 4h · 扫描会话   → 提取长期事实和主题标签
...
可以点按钮选择已有目录，也可以直接在聊天里输入路径——目录不存在时自动创建（`✅ 已新建目录`），已存在则显示 `✓ 已选择目录`。`/agent new` 向导和 `/agent bind` 的所有入口都会校验目录是否有效，再保存配置。
...
- **蒸馏**（`scripts/distill.js`）：心跳任务（默认 4h，信号守卫），更新画像。
- **记忆提取**（`scripts/memory-extract.js`）：心跳任务（默认 4h，闲置守卫），从已完成会话中提取长期事实和主题标签。
