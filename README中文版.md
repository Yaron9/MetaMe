...
MetaMe 的记忆系统完全在后台自动运行——不打扰你，不需要手动操作。三层就够，不硬凑层数。

**第一层 — 长期事实**
闲置时触发记忆巩固：从你的会话中提取关键决策、模式和知识，存入持久化事实库。每次会话启动时语义召回，无感注入。

**第二层 — 历史检索**
如果你提到"昨天那个事"或"上周那个项目"，MetaMe 通过 session 路由、主题标签和 memory recall 回到正确线程、文件和事实。同一个底层 session 直接续聊，不额外注入“恢复摘要”。
...
【第二天早上，恢复会话时】
"继续昨天的" →
  通过 session 检索和 memory recall 回到正确线程/文件，
  不再自动注入伪造的“上次对话摘要”。
```
...
- 维护 daemon 存活状态，轮转日志
- 检测你是否空闲并进入 sleep mode，驱动带守卫的后台任务
...
- Emoji 自动降级为 ASCII（`[OK]`、`[FAIL]`），兼容 GBK
- IPC 使用 Named Pipe 替代 Unix Socket
- `/mac` 命令不可用
- **请用原生 PowerShell/CMD 安装，不建议 WSL**
</details>

<details>
<summary>WSL2 / Linux — systemd 托管</summary>
...
> WSL2 需先开启 systemd：`/etc/wsl.conf` 加入 `[boot]\nsystemd=true`，然后重启 WSL。
...
MetaMe 启动时会自动安装并维护核心 Hook：

- `UserPromptSubmit`（`scripts/hooks/intent-engine.js`）：统一的意图引擎，为团队协作、操作辅助和任务创建等注入上下文辅助提示。
...
无需 npm，直接装进 Claude Code：`claude plugin install github:Yaron9/MetaMe/plugin`

功能完整。插件在 `SessionStart` 时自动拉起 daemon（前提：`daemon.yaml` 已存在）。**不会**自动注册系统服务——重启后需再次打开 Claude 或手动启动 daemon。
