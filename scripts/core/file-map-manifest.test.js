...
function sampleManifest(over = {}) {
  const created = createManifest({
    items: [{
      path: '/home/u/Downloads/x.dmg', size: 100, mtimeMs: 5000.7,
      inode: 42, device: 7, isDirectory: false,
    }],
...
describe('file-map-manifest lifecycle', () => {
  it('creates a well-formed proposed manifest with deterministic shape', () => {
    ...
    assert.equal(m.batch_id, `b-20260718-${'ab'.repeat(16)}`);
    assert.ok(isValidBatchId(m.batch_id));
    assert.equal(m.version, 2);
    assert.equal(m.token, undefined);
    assert.equal(m.token_hash.length, 64);
  ...
  it('TTL expiry and token verification', () => {
    ...
    assert.equal(verifyToken(m, 'ab'.repeat(16)), true);
    assert.equal(verifyToken(m, '0'.repeat(32)), false);
  ...
  it('verifyItemUnchanged detects each drift dimension', () => {
    ...
    const good = { size: 100, mtimeMs: 5000.9, inode: 42, device: 7 }; // sub-ms jitter tolerated via floor
    ...
    assert.equal(verifyItemUnchanged(item, { ...good, device: 8 }).reason, 'device-changed');
  });

  it('manifestPaths and user summary', () => {
    const p = manifestPaths('/base');
    assert.equal(p.proposals, '/base/proposals');
    assert.equal(p.inflight, '/base/inflight');
    ...
    const trashSummary = summarizeForUser(sampleManifest({ method: 'trash' }));
    assert.match(trashSummary, /Finder Trash/);
    assert.doesNotMatch(trashSummary, /cleanup_restore/);
    const redacted = redactManifest(sampleManifest());
    assert.equal(redacted.token_hash, undefined);
    assert.equal(JSON.stringify(redacted).includes('token_hash'), false);
  });
});
