...
describe('daemon-admin-commands /engine', () => {
  it('shows codex default model instead of legacy opus after engine switch', async (t) => {
    const sent = [];
    ...
    const loadConfig = () => yaml.load(fs.readFileSync(configFile, 'utf8')) || {};
    const { handleAdminCommand } = createHandler(
      () => ({ general: [], project: [] }),
      {
        yaml,
        ...
        setDefaultEngine: (engine) => { defaultEngine = engine; },
        getDistillModel: () => 'gpt-5.1-codex-mini',
      }
    );
    const bot = {
    ...
    assert.equal(res.handled, true);
    assert.match(sent[0], /引擎已切换: codex/);
    assert.match(sent[0], /会话模型: gpt-5\.4/);
    assert.doesNotMatch(sent[0], /会话模型: opus/);
  });
});
